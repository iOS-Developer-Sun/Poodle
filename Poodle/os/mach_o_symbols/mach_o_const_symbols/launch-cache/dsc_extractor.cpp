...
#define FA reinterpret_cast<fat_arch*>(base_ptr + (8 + (nfat_archs - 1) * sizeof(fat_arch)))
    ...
    dylib_data.resize((unsigned long)(offsetInFatFile+newSize));
    base_ptr                                    = &dylib_data.front();
...
int dyld_shared_cache_extract_dylibs(const char* shared_cache_file_path, const char* extraction_root_path, const char **dylib_names)
{
	struct stat statbuf;
...
//        dispatch_group_async(group, process_queue, ^{
        ...
        {
            const char *path = it->first;
            const char *start = strrchr(path, '/');
            const char *file = start ? start + 1 : path;

            const char **current = dylib_names;
            bool matched = false;
            while (*current) {
                if (strcmp(file, *current) != 0) {
                    current++;
                    continue;
                } else {
                    matched = true;
            ...
            std::vector<uint8_t> *vec   = new std::vector<uint8_t>((unsigned long)statbuf.st_size);
