...
@interface FMDatabase (BatchAction)

/**
 batch objects update with sql

 @param sql The SQL to be performed, with optional `?` placeholders.
 @param objects Batch objects to be updated, each object is `NSArray` instance to be used when binding values to the `?` placeholders in the SQL statement.
@return `YES` upon success; `NO` upon failure.
 */
- (BOOL)executeBatchUpdate:(NSString *)sql withObjects:(NSArray <NSArray *> *)objects;

/**
 batch objects update with sql
 
 @param sql The SQL to be performed, with optional `?` placeholders.
 @param outErr The SQL executed process error
 @param objects Batch objects to be updated, each object is `NSArray` instance to be used when binding values to the `?` placeholders in the SQL statement.
 @return `YES` upon success; `NO` upon failure.
 */
- (BOOL)executeBatchUpdate:(NSString *)sql error:(NSError * _Nullable*)outErr withObjects:(NSArray <NSArray *> *)objects;
